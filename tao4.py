...
wait = {
    ...
    "pname":{},
    "pro_name":{},
    "tag1":"\n🎉..แท๊คไม่ตอบกรุณารอสักครู่..🎉",
    "tag2":"\n😒..เอาเข้าไปแท๊คทำไมนักหนา..😒",
	"posts":False,
	}
        ...
        if op.type == 26:
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention"] == True:
                     contact = cl.getContact(msg.from_)
                     cName = contact.displayName
                     balas = ["แท๊คทำไม..ก็บอกว่าไม่ว่าง?😎"]
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["kickMention"] == True:
                     contact = cl.getContact(msg.from_)
                     cName = contact.displayName
                     balas = ["กูบอกว่าอย่าแท๊คเยอะไง",cName + " Ngapain Ngetag?",cName + " Nggak Usah Tag-Tag! Kalo Penting Langsung Pc Aja","-_-","Alin lagi off", cName + " Kenapa Tag saya?","SPAM PC aja " + cName, "Jangan Suka Tag gua " + cName, "Kamu siapa " + cName + "?", "Ada Perlu apa " + cName + "?","Tenggelamkan tuh yang suka tag pake BOT","Tersummon -_-"]
                     ret_ = "[ข้อความ อัตโนมัติ] " + random.choice(balas)
                     name = re.findall(r'@(\w+)', msg.text)
                     mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                     mentionees = mention['MENTIONEES']
...
#======================================================#
                ...
                cl.sendText(msg.to,"ข้อความแท๊คล่าสุดคือ\n\n" + str(wait["tag1"]))
